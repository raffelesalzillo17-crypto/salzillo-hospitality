...
- **10/09/2026**: fasi 0-5 fatte in autonomia (Raffaele: "facciamo tutto quello che manca"). **0** GitHub. **1** Neon + schema (25 tabelle). **2** query di lettura. **3** import verificato. **4** sync giornaliero NON distruttivo (colonna `origine`: i record creati in /nuovo sopravvivono al re-sync dal foglio). **5**: `/nuovo` completo — login utente+password con ruoli e permessi per immobile; dashboard con "cosa manca"; calendario stile Airbnb; prenotazioni (crea con calcolo importi da regime alloggio + fee contratto, modifica, cancella); ospiti; gestione anagrafica (proprietari/immobili/alloggi con regime, pulizia, wifi, flag trasmissione, imposta soggiorno); spese; scadenze; rendiconto proprietario (web + PDF, con imposta soggiorno); documenti (conferma prenotazione, preventivo, contratto di gestione); generatore file Sinfonia (bozza). **Il sistema che Raffaele usa resta intatto.** **Ancora da fare**: il "flip" (Raffaele: parallelo ~1 mese, poi spegnere il foglio con possibilità di riaccenderlo subito), spostare Calendar/Drive su salzillohospitality@gmail.com (OAuth di Raffaele), collegare GitHub↔Vercel, evento Google Calendar sulle prenotazioni create in /nuovo, affinare Sinfonia col portale reale, imposta di soggiorno con esenzioni.
