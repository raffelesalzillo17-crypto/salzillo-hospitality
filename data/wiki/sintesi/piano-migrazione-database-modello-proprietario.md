...
- **10/09/2026**: fasi 0-5 avanzate in autonomia. **0** GitHub. **1** Neon + schema 21 tabelle. **2** strato query letture. **3** import verificato al centesimo. **4** cron `sync-db` giornaliero (piano Hobby, no orario) che riallinea il db al foglio + bottone manuale — il foglio resta la fonte viva, niente dual-write. **5 (parziale)** interfaccia `/nuovo` (key-gated, grafica Motore Rafilu): dashboard con "cosa manca", **calendario stile Airbnb**, liste prenotazioni/ospiti, vista immobili con proprietario/regime, **rendiconto proprietario mensile con cascata + PDF**. Tutto in parallelo, il sistema che Raffaele usa è intatto. **Da fare**: il "flip" vero (db come fonte, richiede settimana di osservazione), scritture in `/nuovo`, login rifatto, spostamento Calendar/Drive su salzillohospitality@gmail.com (serve OAuth di Raffaele), generazione altri documenti (preventivo, conferma prenotazione, contratto gestione), imposta di soggiorno, notifiche schedine con scadenza 6h/24h.
