...
import { inviaTelegram, testoPulizieDomani } from '@/lib/telegramDigest';

// Promemoria pulizie di domani. Dal 15/09/2026 non è più schedulato da solo in vercel.json —
...
export async function GET(req: NextRequest) {
  ...
  try {
    const text = await testoPulizieDomani();
    if (!text) return NextResponse.json({ ok: true, sent: false, note: 'Nessuna pulizia da fare domani', dryRun });
